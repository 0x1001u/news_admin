    ...
    <el-card class="box-card !bg-gray-800 !border-gray-700 !text-gray-50 rounded-lg shadow-md">
        <template #header>
            <div class="card-header flex justify-between items-center">
                <h2 class="text-xl font-semibold text-primary-500">标签管理</h2>
                <el-button type="primary" @click="openTagDialog()">新增标签</el-button>
            </div>
        </template>

        <div v-loading="loading" element-loading-text="加载中..." class="min-h-64">
            <el-table :data="tags" style="width: 100%" class="!bg-gray-800 !text-gray-50 !border-gray-700 rounded-md"
            ...
            >
                ...
                <el-table-column label="操作" width="180">
                    <template #default="scope">
                        <el-button size="small" @click="openTagDialog(scope.row)">编辑</el-button>
                        <el-popconfirm
                            title="确定要删除此标签吗？"
            ...
            <el-empty v-if="!tags.length && !loading" description="暂无标签数据" class="text-gray-400" />
        </div>

        <el-dialog v-model="tagDialogVisible" :title="currentTagId ? '编辑标签' : '新增标签'" width="500px"
            ...
            <el-form :model="tagForm" label-width="80px" class="p-4">
                <el-form-item label="名称">
                    <el-input v-model="tagForm.name" class="!bg-gray-700 !text-gray-50"></el-input>
                </el-form-item>
                <el-form-item label="别名">
                    <el-input v-model="tagForm.slug" class="!bg-gray-700 !text-gray-50"></el-input>
                </el-form-item>
            </el-form>
            <template #footer>
                <div class="dialog-footer px-4 py-2 border-t border-gray-700 flex justify-end space-x-2">
                    <el-button @click="tagDialogVisible = false">取消</el-button>
                    <el-button type="primary" @click="submitTagForm">保存</el-button>
                </div>
            </template>
        </el-dialog>
    </el-card>
...
const submitTagForm = async () => {
    try {
        if (currentTagId.value) {
            await apiClient.put(`/tags/${currentTagId.value}`, tagForm);
            ElMessage.success('标签更新成功！');
        } else {
            await apiClient.post('/tags/', tagForm);
